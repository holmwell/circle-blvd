...
module.exports = function (dbParam) {
	db = dbParam;
	settingsCache = require('circle-blvd/settings')(db);
	return {
		newStory: newStory,
...
function newStory (story, sender, req, callback) {
	...
	db.users.findByCircleAndName(story.projectId, story.owner, guard(callback, function (owner) {
		// Use notification email addresses
		...
		sendStoryNotification(params, guard(callback, function (response) {
...
var sendStoryNotification = function (params, callback) {
	...
	settingsCache.get(function (err, settings) {
		if (err) {
			return callback({
				status: 501,
...
function newComment (params, req) {
...
function forgotPassword (params, req, callback) {
...
function invitation (params, req, callback) {
