...
module.exports = function (db) {
	var settingsCache = require('circle-blvd/settings')(db);

	var checkCircleLimit = function (req, res, next) {
		checkLimit(next);

		function checkLimit (callback) {
			settingsCache.get(guard(res, function (settings) {
		...
		function checkSettings(settings, callback) {
			...
			db.circles.count(guard(res, function (count) {
				if (count >= limit) {
					res.status(403).send("Sorry, our computers have reached their circle creation limit," +
	...
	var checkMemberLimit = function (req, res, next) {
		...
		function checkSettings(settings, callback) {
			...
			db.users.count(guard(res, function (count) {
				if (count >= limit) {
					res.status(403).send("Sorry, our computers have reached their member creation limit," +
	...
	var checkUserCircleLimit = function (req, res, next) {
		db.circles.countByUser(req.user, guard(res, function (count) {
			...
			if (count >= maxCircleCount) {
				return res.status(403).send("Sorry, you can only create " + maxCircleCount + " circles.");
			}
	...
	var checkUserStoryLimit = function (circleId, next) {
		...
		function getCircle (callback) {
			db.docs.get(circleId, guard(callback, function (circle) {
		...
		function checkStoryLimit (callback) {
			async.parallel([settingsCache.get, getCircle], guard(callback, function (results) {
		...
		function checkSettings(settings, circle, callback) {
			...
			db.stories.countByCircleId(circleId, guard(callback, function (count) {
				if (count >= limit) {
	...
	var checkArchiveLimit = function (circleId, next) {
		...
		function checkSettings(settings, circle, callback) {
			...
			db.archives.countByCircleId(circleId, guard(callback, function (count) {
				if (count >= limit) {
					// TODO: Add validation-specific instructions, after 
					// we have an account validation mechanism.
