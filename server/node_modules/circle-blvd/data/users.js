...
module.exports = function (couch) {
	var things = require('./couch/users.js')(couch);
	...
	var findNamesByCircleId = function (circleId, callback) {
		couch.users.findNamesByCircleId(circleId, guard(callback, function (names) {
	...
	var addUser = function(name, email, password, memberships, isReadOnly, success, failure) {
		...
		var addUserToDatabase = function (user, password) {
			...
			couch.users.add(user, password, guard(failure, function (body) {
				user._id = body.id;
				user._rev = body.rev;
				success(user);	
			}));
	...
	var removeUser = function(user, success, failure) {
		...
		couch.users.remove(user, guard(failure, function (body) {
			return success();
		}));
	};
	...
	var updateUser = function(user, success, failure) {
		...
		user = normalizeUser(user);
		couch.users.update(user, guard(failure, function (body) {
			user._rev = body.rev;
			success(user);
		}));
	};
	...
	var updateUserPassword = function(user, password, success, failure) {
		...
		couch.users.updatePassword(user, password, 
			guard(failure, success));
	};
	...
	var addMembership = function (member, circleId, callback) {
		...
		findUserByEmail(member.email, guard(callback, function (userAccount) {
			if (userAccount) {
	...
	var removeMembership = function (reqUser, circleId, callback) {
		findUserById(reqUser.id, guard(callback, function (user) {
	...
	var updateUserGroups = function (member, circleId, groups, callback) {
		findUserById(member.id, guard(callback, function (user) {
