...
module.exports = function (couch) {
	couch.stories = require('./couch/stories.js')(couch);
	couch.listMeta = require('./couch/list-meta.js')(couch, couch.stories);

	var ee = new events.EventEmitter();

	var isFirstStoryCreated = function (meta) {
		return !meta.firstStoryId;
	};
	...
	var getFirstStory = function (projectId, callback) {
		...
		couch.listMeta.byListId(projectId, guard(callback, function (meta) {
			couch.docs.get(meta.firstStoryId, guard(callback, function (story) {
				var firstStory = getModelStory(story, meta);
	...
	var getNextMeeting = function (projectId, callback) {
		...
		couch.listMeta.byListId(projectId, guard(callback, function (meta) {
			couch.docs.get(meta.nextMeetingId, guard(callback, function (story) {
				var nextMeeting = getModelStory(story, meta);
				callback(null, nextMeeting);
			}));
		}));
	};

	function getModelStory (story, listMeta) {
		var modelStory = {
			id: story._id,
			projectId: story.projectId,
			listId: story.listId,

			// Get the nextId and first-story stuff
			// from our list-meta, as list-meta is 
			// what determines our story order.
			nextId: listMeta.nextIds[story._id],
			isFirstStory: listMeta.firstStoryId === story._id,
	...
	var findStoriesByListId = function (listId, callback) {
		...
		var prepareStories = function (dbStories, listMeta) {
			var stories = {};
			dbStories.forEach(function (story, index, array) {

				var modelStory = getModelStory(story, listMeta);
		...
		couch.stories.findByListId(listId, guard(callback, function (dbStories) {
			couch.listMeta.byListId(listId, guard(callback, function (listMeta) {
				prepareStories(dbStories, listMeta);
			}));
		}));
	};

	var findStoriesByNextId = function (nextId, meta, callback) {
		var storyIds = [];

		for(var id in meta.nextIds) {
			if (meta.nextIds[id] === nextId) {
				storyIds.push(id);
			}
		}

		couch.stories.findMany(storyIds, guard(callback, function (body) {
			callback(null, body.rows);
		}));
	};

	var findStoryByNextId = function (nextId, meta, callback) {
		var storyId = null;
		...
		couch.stories.findById(storyId, callback);
	};
	...
	var postInsertStory = function (story, nextId, success, failure) {
		if (story._id === nextId) {
			return failure({
				message: "Post-insert: Circular reference not allowed"
			});				
		}

		var saveMeta = function (meta) {
			couch.listMeta.update(meta, function (err) {
				if (err) {
					failure(err);
				}
				else {
		...
		var listId = story.listId || story.projectId;
		couch.listMeta.byListId(listId, function (err, meta) {
			if (err) {
				console.log("Update-story-metadata: Failure to find list meta");
				return failure(err);
			}

			meta.nextIds[story._id] = nextId;

			if (!meta.firstStoryId) {
				meta.firstStoryId = story._id;
				return saveMeta(meta);
			}

			if (meta.firstStoryId === nextId) {
				meta.firstStoryId = story._id;
				return saveMeta(meta);
			}
			else {
				...
				findStoryByNextId(nextId, meta, guard(failure, function (previousStory) {
					if (meta.nextIds[previousStory._id] === story._id) {
						// Fine.
					}
					else {
						meta.nextIds[previousStory._id] = story._id;
					}
					saveMeta(meta);
				}));
			}
	...
	var getNextIdForInsert = function (story, proposedNextId, callback) {
		...
		var foundNextId = function (nextId) {
			callback(null, nextId || getLastId(story));
		};
		...
		couch.listMeta.byListId(listId, guard(callback, function (meta) {
			if (!meta.firstStoryId && !proposedNextId) {
				if (story.isNextMeeting) {
					// Ok, we're actually the first story created.
					foundNextId(getLastId(story));
				}
				else {
					countByListId(story.listId || story.projectId, guard(failure, function (count) {
						...
						// of the backlog.
						getFirstStory(story.listId || story.projectId, guard(failure, function (firstStory) {
							if (!firstStory) {
								return foundNextId(getLastId(story));
							}
				...
				// point to.
				findStoriesByNextId(proposedNextId, meta, function (err, conflicts) {
					if (err) {
	...
	var processStory = function (story, callback) {
		...
		getNextIdForInsert(story, proposedNextId, function (err, nextId) {
			...
			couch.docs.update(story, function (err, processedStory) {
				...
				postInsertStory(processedStory, nextId, 
	...
	var handleAddThing = function (err, thing, callback) {
		...
		couch.stories.addToQueue(story, function (err, body) {
			...
			processStory(story, function (err, newStory) {
				...
				var listId = newStory.listId || newStory.projectId;
				couch.listMeta.byListId(listId, guard(callback, function (meta) {
					var model = getModelStory(newStory, meta);
					ee.emit(thing.id, err, model);
					return callback();
				}));
			});
		});
	...
	var oldMoveStory = function (story, newNextId, success, failure) {
		...
		var saveMeta = function (meta) {
			couch.listMeta.update(meta, function (err, response) {
				if (err) {
		...
		couch.stories.findById(story.id, guard(failure, function (storyToMove) {
			if (!storyToMove) {
				return failure({
			...
			var listId = storyToMove.listId || storyToMove.projectId;
			couch.listMeta.byListId(listId, guard(failure, function (meta) {

				if (meta.nextIds[storyToMove.id] === newNextId) {
					// We're already there. Nothing to do.
					return success(story);
				}

				findStoryByNextId(storyToMove.id, meta, guard(failure, function (preMovePreviousStory) {
					findStoryByNextId(newNextId, meta, guard(failure, function (storyC) {
						couch.stories.findById(newNextId, guard(failure, function (storyE) {
							...
							couch.stories.findById(meta.nextIds[storyToMove.id], guard(failure, function (preMoveNextStory) {
								// This might happen if there is a ton of activity, I guess.
								if (!preMoveNextStory && meta.nextIds[storyToMove.id] !== lastId) {
									return failure({
								...
								couch.stories.findById(meta.firstStoryId, 
									...
									if (preMovePreviousStory) {
										// base case
										meta.nextIds[preMovePreviousStory.id] = meta.nextIds[storyToMove.id];
										...
										meta.firstStoryId = preMoveNextStory.id;
									}
									
									meta.nextIds[storyToMove.id] = newNextId;
									/// storyToMove.nextId = newNextId;

									if (storyC) {
										// base case
										meta.nextIds[storyC.id] = storyToMove.id;
										...
										meta.firstStoryId = storyToMove.id;
	...
	var oldRemoveStory = function (story, success, failure) {
		...
		couch.stories.findById(story.id, guard(failure, function (storyToRemove) {
			if (!storyToRemove) {
			...
			couch.listMeta.byListId(listId, guard(failure, function (meta) {
				...
				findStoryByNextId(storyToRemove.id, meta, guard(failure, function (previousStory) {
					couch.stories.findById(meta.nextIds[storyToRemove.id], guard(failure, function (nextStory) {
						var lastId = getLastId(storyToRemove);
						if (meta.nextIds[storyToRemove.id] === lastId) {
							nextStory = {};
							nextStory.id = lastId;
						}
						else if (!nextStory) {
						...
						var actuallyRemove = function (storyToRemove) {
							delete meta.nextIds[storyToRemove.id];
							couch.listMeta.update(meta, guard(failure, function (meta) {
								couch.stories.remove(storyToRemove, guard(failure, success));
							}));
						};

						actuallyRemove(story);

						// if (storyToSave.id === lastId && storyToSave.isFirstStory) {
						// 	// If the story 'next to' the story to remove is both
						// 	// the first and the last story, that means it doesn't
						// 	// exist, and we're actually removing the final story
						// 	// in the list, which can happen with checklists.
						// 	console.log("Removing final story: " + lastId);
						// 	actuallyRemove(story);
						// }
						// else {
						// 	couch.stories.update(storyToSave, guard(failure, function () {
						// 		actuallyRemove(story);
						// 	}));
						// }
					}));
	...
	var removeStory = function (story, callback) {
		...
		var afterEnqueue = guard(callback, function () {
			...
			// our thing, we'll emit an event named
			// thing.id. and then we'll callback.
			ee.once(thing.id, guard(callback, function () {
				callback();
			}));
		});
	...
	var markStoryOwnerNotified = function (story, success, failure) {
		couch.stories.findById(story.id, guard(failure, function (storyToSave) {
	...
	return {
		add: addStory,                      // uses list-meta
		markOwnerNotified: markStoryOwnerNotified, // does not need list-meta
		move: moveStory,
		moveBlock: moveBlock,
		remove: removeStory,
		find: findStoriesByListId,          // uses list-meta
		findByListId: findStoriesByListId,  // uses list-meta
		getFirstByProjectId: getFirstStory, // uses list-meta
		getNextMeetingByProjectId: getNextMeeting, // uses list-meta
		save: saveStory,
		fix: updateStory,
