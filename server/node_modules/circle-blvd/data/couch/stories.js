...
module.exports = function (couch) {
	...
	var removeStory = function(story, callback) {
		findStoryByIds(story.projectId, (story.id || story._id), guard(callback, function (body) {
			if (story === null) {
			...
			database.destroy(story._id, story._rev, callback);
		}));
	};
	...
	var nextStoryInQueue = function (callback) {
		var params = {};
		params.limit = 1;
		couch.view("queue/byTimestamp", params, guard(callback, function (rows) {
			if (rows.length > 0) {
	...
	var findStoryById = function (storyId, callback) {
		couch.view("stories/byId", {key: storyId}, guard(callback, function (rows) {
			if (rows.length > 0) {
				callback(null, rows[0]);
			}
			else {
	...
	var findStoriesByListId = function (listId, callback) {
		...
		couch.view("stories/byListId", options, callback);
	};
	...
	var storiesTransaction = function (stories, callback) {
		...
		var startTransaction = function (oldStories, newStories) {
			...
			database.bulk(bulkEnter, options, guard(callback, function (response) {
				...
				database.bulk(bulkDoc, options, guard(callback, function (response) {
					for (var docIndex in response) {
						if (!response[docIndex].ok) {
							return callback({
		...
		database.fetch(initialQuery, guard(callback, function (body) {
