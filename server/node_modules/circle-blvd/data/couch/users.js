...
module.exports = function () {
	...
	var findUsersByCircleId = function (circleId, callback) {
		...
		var users = [];
		var uniqueUsers = {};
		couch.view("users/byCircle", options, guard(callback, function (viewUsers) {
	...
	var findUserByCircleAndName = function (circleId, name, callback) {
		...
		findUsersByCircleId(circleId, guard(callback, function (usersInCircle) {
			var users = [];
	...
	var findUsersById = function (idList, callback) {
		...
		if (idList.length > 0) {
			query["keys"] = idList;
			database.fetch(query, guard(callback, function (body) {
	...
	var addUser = function(user, password, callback) {
		user.type = "user";
		database.insert(user, guard(callback, function () {
	...
	var removeUser = function (user, callback) {
		findPasswordById(user.id, guard(callback, function (pass) {
			// TODO: Make this a transaction.
			database.destroy(pass._id, pass._rev, guard(callback, function (body) {
	...
	var updateUser = function(user, callback) {
		findUserById(user.id, guard(callback, function (body) {
	...
	var updateUserPassword = function (user, password, callback) {
		findPasswordById(user.id, guard(callback, function (body) {
			var pass = createPasswordDoc(user.id, password);
			pass._id = body._id;
			pass._rev = body._rev;
	...
	var getAllUsers = function(callback) {
		couch.view("users/byId", callback);
	};

	var countUsers = function (callback) {
		couch.view("users/count", guard(callback, function (rows) {
			var count = 0;
	...
	var adminCount = function (callback) {
		couch.view("users/administrative", guard(callback, function (rows) {
			var count = 0;
			if (rows && rows.length > 0) {
				count = rows[0];
			}
