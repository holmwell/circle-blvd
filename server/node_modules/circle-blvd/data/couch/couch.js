...
var couch = function(dbConfig) {
	...
	databaseName = dbConfig.name || 'circle-blvd';
	...
	var updateDoc = function (doc, callback) {
		database.get(doc._id, guard(callback, function (body) {
			doc._rev = body._rev;

			database.insert(doc, guard(callback, function (body) {
				doc._rev = body.rev;
				callback(null, doc);
			}));
		}));
	...
	var findOneByKey = function(viewName, key, callback) {
		...
		getView(viewName, options, guard(callback, function (rows) {
			var doc = null;
